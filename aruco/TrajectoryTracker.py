...
class TrajectoryTracker:
    ...
    def updateTrajectory(self, aligned_frame, detectorResult):
        ...
        timestamp = aligned_frame[4]
        depth_frame = np.frombuffer(depth_raw[6], np.uint8).reshape(aligned_frame[1], aligned_frame[0], 3)
        depth_intrinsics = [aligned_frame[0],aligned_frame[1],
        #.profile.as_video_stream_profile().intrinsics
    ...
    def _getCoordinate(self, depth_intrinsics, depth_frame, markerPos):
        ...
        depth = depth_frame.get_distance(cX, cY) ##float
